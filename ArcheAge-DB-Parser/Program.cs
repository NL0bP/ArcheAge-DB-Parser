...
namespace ArcheAge_DB_Parser
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Parser.parse("game5", "ru.json", 36); // для дампа памяти в x32dbg
            //Parser.parse("game6", "de.json");
            //Parser.parse("game6", "de.json", 36); // для дампа памяти в x32dbg
            //Parser.parse("game7", "fr.json");
            //Parser.parse("game7", "fr.json", 36); // для дампа памяти в x32dbg
            //Parser.parse("game", "ja.json");
            //Parser.parse("game", "zh_tw.json");
            //Parser.parse("game8", "main_db.json"); // 3+, 5+
            Parser.parse("game8", "main_db.json", 36); // 3+, 5+ // для дампа памяти в x32dbg
